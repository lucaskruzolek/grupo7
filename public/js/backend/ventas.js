...
function applyFilters() {
    ...
    const paymentMethod = filterPago.value;

    filterOrders(query, status, paymentMethod);
}
 ...
 */
function filterOrders(query, status, paymentMethod) {
    const tableRows = document.querySelectorAll('.orders-table tbody tr');
    const detailContent = document.getElementById('detail-content');
    const detailEmpty = document.getElementById('detail-empty');
    ...
    tableRows.forEach(row => {
        ...
        const matchStatus = (status === 'all') || (estadoVenta === status);
        const matchPayment = (paymentMethod === 'all') || (pagoId === paymentMethod);

        if (matchSearch && matchStatus && matchPayment) {
            row.classList.remove('d-none');
        } else {
            row.classList.add('d-none');
            // If the active row gets filtered out, clear details pane
